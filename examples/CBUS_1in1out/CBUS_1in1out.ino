...
#include <CBUSUNOR4.h>              // CAN controller and CBUS class for UNO R4
...
CBUSUNOR4 CBUS(&module_config);     // CBUS object
...
void setupCBUS() {
  ...
  // set size of message receive buffer
  CBUS.setNumBuffers(32);
  ...
  if (!CBUS.begin()) {
    Serial << F("> CBUS init fail") << endl;
  } else {
    Serial << F("> CBUS init ok") << endl;
  }
  ...
  // test CAN message
  test_msg.id = 12345;
  memcpy(test_msg.data, test_data, 5);
}
...
void loop() {
  ...
  static bool lphase = true;
  static unsigned long ltimer = millis();

  if (millis() - ltimer >= 500) {
    ltimer = millis();
    lphase = !lphase;
    digitalWrite(LED_BUILTIN, lphase);
  }
...
void printConfig(void) {
  ...
  // copyright
  Serial << F("> © Duncan Greenwood (MERG M5767) 2019-2025") << endl;
}
  ...
  if (Serial.available()) {
    ...
    switch (c) {
      ...
      case 's':                     // send a standard 11-bit CAN message
        test_msg.len = 5;
        r = CBUS.sendMessage(&test_msg, false, false);
        Serial << "transmission of standard frame returns: " << (r ? "ok" : "fail");
        break;

      case 'x':                     // send an extended 29-bit CAN message
        test_msg.len = 5;
        r = CBUS.sendMessage(&test_msg, false, true);
        Serial << "transmission of extended frame returns: " << (r ? "ok" : "fail");
        break;

      case 'r':                     // send a remote transfer request (RTR) frame
        test_msg.len = 0;
        r = CBUS.sendMessage(&test_msg, true, false);
        Serial << "transmission of remote frame returns: " << (r ? "ok" : "fail");
        break;
